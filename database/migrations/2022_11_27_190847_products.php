<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    /**
     * Run the migrations.
     *
     * @return void
     */
    public function up()
    {
        Schema::create('products', function (Blueprint $table) {
            $table->increments('id');
            $table->integer('status')->nullable();
            $table->integer('categoryId')->nullable();
            $table->text('title_en')->nullable();
            $table->text('title_de')->nullable();
            $table->text('title_ru')->nullable();
            $table->text('link_en')->nullable();
            $table->text('link_de')->nullable();
            $table->text('link_ru')->nullable();
            $table->text('description_en')->nullable();
            $table->text('description_de')->nullable();
            $table->text('description_ru')->nullable();
            $table->text('keywords_en')->nullable();
            $table->text('keywords_de')->nullable();
            $table->text('keywords_ru')->nullable();
            $table->text('content_en')->nullable();
            $table->text('content_de')->nullable();
            $table->text('content_ru')->nullable();
            $table->integer('stock')->nullable();
            $table->decimal('price', 10,2)->nullable();
            $table->decimal('discountedPrice', 10,2)->nullable();
            $table->integer('discount')->nullable();
            $table->decimal('finalPrice', 10,2)->nullable();
            $table->string('currency', 10)->nullable();
            $table->integer('orderForm')->nullable();
            $table->string('feature_de')->nullable();
            $table->string('feature_en')->nullable();
            $table->string('feature_ru')->nullable();
            $table->string('type_de')->nullable();
            $table->string('type_en')->nullable();
            $table->string('type_ru')->nullable();
            $table->string('formTitle_de')->nullable();
            $table->string('formTitle_en')->nullable();
            $table->string('formTitle_ru')->nullable();
            $table->integer('comments')->nullable();
            $table->integer('featureId')->nullable();
            $table->string('coverImage', 50)->nullable();
            $table->datetime('lastDate')->nullable();
            $table->integer('lastAdminId')->nullable();
            $table->timestamps();
        });
    }

    /**
     * Reverse the migrations.
     *
     * @return void
     */
    public function down()
    {
        Schema::dropIfExists('products');
    }
};
